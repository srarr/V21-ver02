...
update_roadmap_checkboxes() {
    ...
    case "$phase" in
        "1.3")
            # Update Phase 1.3: Orchestrator section (lines 58-86)
            sed -i '58,86s/- \[ \]/- [x]/g' "$roadmap_file"
            ;;
        "1.4")
            # Update Phase 1.4: Frontend section (lines 88-108)
            sed -i '88,108s/- \[ \]/- [x]/g' "$roadmap_file"
            ;;
        "1.5")
            # Update Phase 1.5: Integration & Testing section (lines 109-137)
            sed -i '109,137s/- \[ \]/- [x]/g' "$roadmap_file"
            ;;
        "1.6")
            # Update Phase 1.6: UI Iteration Safety section (lines 138-338)
...
detect_current_phase() {
    ...
    if [ -f "$TODOS_FILE" ]; then
        if command -v jq &> /dev/null; then
            PHASE=$(jq -r '.currentPhase // "1.3"' "$TODOS_FILE")
            echo "$PHASE"
            return
        else
            # Fallback parsing without jq
            PHASE=$(grep '"currentPhase"' "$TODOS_FILE" | sed 's/.*"currentPhase": *"\([^"]*\)".*/\1/')
            if [ -n "$PHASE" ]; then
                echo "$PHASE"
                return
            fi
    ...
    if git log --oneline -10 2>/dev/null | grep -q "Phase"; then
        PHASE=$(git log --oneline -10 | grep -o "Phase [0-9]\.[0-9]" | head -1 | sed 's/Phase //')
        echo "$PHASE"
    else
...
update_todos_json() {
    ...
    if [ -f "$TODOS_FILE" ]; then
        ...
        sed -i "/\"$phase\":/,/\"status\":/ s/\"status\":.*\"[^\"]*\"/\"status\": \"$status\"/" "$TODOS_FILE"
...
detect_completed_tasks() {
    ...
    if git diff --cached --name-only | grep -q "_test\\.go$\|_test\\.js$\|_test\\.ts$"; then
        echo "tests"
    fi
    
    # Check for new endpoints
    if git diff --cached | grep -q "router\\..*(\|app\\..*(\|r\\..*(\"; then
        echo "endpoints"
    fi
...
main() {
    ...
    if [ -f "$PROJECT_ROOT/scripts/todo-persist.js" ]; then
        node "$PROJECT_ROOT/scripts/todo-persist.js" init
    fi
    ...
    if git status --porcelain | grep -q "^[AM].*orchestrator/.*\\.ts\|^[AM].*graph\\.ts\|^[AM].*nodes/"; then
    ...
    if git status --porcelain | grep -q "^[AM].*web/.*\\.svelte\|^[AM].*web/.*\\.ts\|^[AM].*web/.*\\.js"; then
    ...
    if [ -f "$TODOS_FILE" ]; then
        ...
        for phase in "1.3" "1.4" "1.5" "1.6"; do
            if command -v jq &> /dev/null; then
                phase_status=$(jq -r ".phases[\"$phase\"].status // \"pending\"" "$TODOS_FILE")
            else
                phase_status=$(grep -A3 "\"$phase\":" "$TODOS_FILE" | grep "status" | sed 's/.*"status": *"\([^"]*\)".*/\1/')
            fi
            
            if [ "$phase_status" = "completed" ]; then
                update_roadmap_checkboxes "$phase" "completed"
            fi
        done
    fi
    ...
    if [ -f "$PROJECT_ROOT/PHASE1-TASKS.md" ]; then
        PHASE_SECTION=$(sed -n "/Phase $CURRENT_PHASE:/,/^## /p" "$PROJECT_ROOT/PHASE1-TASKS.md")
        ...
        if [ "$UNCHECKED" -eq 0 ] && [ -n "$PHASE_SECTION" ]; then
            ...
            update_roadmap_checkboxes "$CURRENT_PHASE" "completed"
            
            # Update CONTINUITY.md
            if [ -f "$PROJECT_ROOT/CONTINUITY.md" ]; then
                sed -i "s/Current Status: .*/Current Status: Phase $CURRENT_PHASE COMPLETED/" "$PROJECT_ROOT/CONTINUITY.md"
    ...
    if [ -f "$TODOS_FILE" ]; then
        if command -v jq &> /dev/null; then
            ...
            TOTAL=$(jq '[.phases | to_entries[] | .value.tasks | length] | add // 0' "$TODOS_FILE")
            COMPLETED=$(jq '[.phases | to_entries[] | .value.tasks[] | select(.status == "completed")] | length' "$TODOS_FILE")
        else
            # Fallback counting method
            TOTAL=$(grep '"id": "1\.' "$TODOS_FILE" | wc -l)
            COMPLETED=$(grep '"status": "completed"' "$TODOS_FILE" | wc -l)
        fi
        
        if [ "$TOTAL" -gt 0 ]; then
            RATE=$(echo "scale=1; $COMPLETED * 100 / $TOTAL" | bc 2>/dev/null || echo "0")
            echo -e "${GREEN}✓${NC} Updated statistics: $COMPLETED/$TOTAL ${RATE}%"
        fi
    fi
